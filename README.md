...
Если в выбранном месте уже установлена другая версия, установщик покажет
обнаруженную и новую версии и спросит подтверждение перед перезаписью.
...
и номер версии. Актуальный commit ветки определяется через GitHub API. Если
доступна новая версия, `add_key` скачивает `install.sh` из того же commit и
запускает его для текущего каталога установки. Предыдущая версия сохраняется
рядом с установленным файлом под именем `add_key.sh.install-backup`.
